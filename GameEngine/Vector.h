...
/// small in "float" percision. 
	#ifndef VERY_SMALL//if varible VERYSMALL
	#define VERY_SMALL 1.0e-7f//make a VERYSMALL make it = 1.0...
	#endif
	...
	struct Vec3 {
		...
		inline void Load( float _x, float _y, float _z ) {//Just like set
			x = _x; y = _y; z = _z; 
		}

		/// Here's a set of constructors
		inline Vec3( float s = float(0.0) ){//sets them all to 0
			Load(s,s,s);
		}

		inline Vec3( float _x, float _y, float _z ){//now pts them in a vec3
			Load(_x,_y,_z);
		}
		...
		inline Vec3( const Vec3& v ) { 
			...
			Load(v.x,v.y,v.z);//adds to itself 
		}
		...
		inline Vec3& operator = (const Vec3& v){//overloading = 
			Load(v.x,v.y,v.z); 
			return *this;
		}
		...
		/// Add two Vec3s
		inline const Vec3 operator + ( const Vec3& v ) const { //the end cnst eans the 2 adding vectors dont change so the result is a third new one
			return Vec3( x + v.x, y + v.y, z + v.z ); 
		}

		/// Add a Vec3 to itself
		inline Vec3& operator += ( const Vec3& v ){ //saying you are going to change the vector(this=your xyz)
			x += v.x;  
			y += v.y;  
			z += v.z;  
			return *this; 
